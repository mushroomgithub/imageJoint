...
int main(int argc,char* argv[])
{
	...
	const char* filename2="./capture2.avi";
        ...
        CvVideoWriter *writer1 =cvCreateVideoWriter(filename1, CV_FOURCC('M','J','P','G'), 25, cvSize(frame1->width,frame1->height));//create writer 
        CvVideoWriter *writer2 =cvCreateVideoWriter(filename2, CV_FOURCC('M','J','P','G'), 25, cvSize(frame2->width,frame2->height));//create writer 
        int n = 2;
        while(1)
    {
                frame1 = cvQueryFrame( capture1  );
